...
MODEL_PATH = os.path.join(PROJECT_PATH, "ressources", "weights",
"dnn_mnist.h5")
# Load the UI File
...
class Painter(QtWidgets.QWidget):
 ...
 if (self.LastPos.x != self.MouseLoc.x) and (self.LastPos.y !=
self.MouseLoc.y):
 self.LastPos = Point(event.x(), event.y())
 self.ParentLink.DrawingShapes.NewShape(self.LastPos,
self.ParentLink.ShapeNum) # type: ignore
 self.repaint()
 # Mose Up Event
...
type: ignore
 ...
 painter.setPen(pen)
 painter.drawLine(T.location.x, T.location.y, T1.location.x,
T1.location.y)
# Main UI Class
